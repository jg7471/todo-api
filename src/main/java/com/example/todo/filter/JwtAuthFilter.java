...
public class JwtAuthFilter extends OncePerRequestFilter {
    //OncePerRequestFilter : request 가 한번씩 들어올 때마다 실행되는 filter
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if (token != null) { //단축키 ifn
                //토큰 서명 위조 검사와 토큰을 파싱해서 클레임(토큰 내용)을 얻어내는 작업.
                ...
                //우리는 Role 타입의 필드 하나만으로 권한을 체크하기 때문에 하나만 add, 여러개라면 여러개 add 하세요
    ...
    private String parseBearerToken(HttpServletRequest request) { //내부에서만 쓰니 private
        //요청 헤더에서 토큰 꺼내오기
        //--content-type: application/json
        //--Authorization: Bearer abcdefg123456...

        String bearerToken = request.getHeader("Authorization");//request야 header 정보 꺼내오기

        //요청 헤더에서 가져온 토큰은 순수 토큰 값이 아닌
        //앞에 Bearer가 붙어 있으니 이것을 제거하는 작업
        if (StringUtils.hasText(bearerToken)
                && bearerToken.startsWith("Bearer")) {
            return bearerToken.substring(7); //'bearer ' 이후 얻기
        }
        return null;
    }
