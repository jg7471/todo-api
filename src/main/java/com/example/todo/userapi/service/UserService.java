...
public class UserService {
    ...
    public LoginResponseDTO authenticate(final LoginRequestDTO dto) {
        ...
        //리프레시 토큰은 수며이 김(long)(2~3주 ~ 2~3개월)
        //DB에 저장해 놓고, 새로운 엑세스 토큰 요청 때마다 만료일을 조회해서 비교
    ...
    public LoginResponseDTO promoteToPremium(TokenUserInfo userInfo) {
        User user = userRepository.findById(userInfo.getUserId()) //jpa기본 제공 메소드
    ...
    public String uploadProfileImage(MultipartFile profileImage) throws IOException {
        //루트 드렉토리가 실존하는지 확인 후 존재하지 않으면 생성
        File rootDir = new File(uploadRootPath);
        if (!rootDir.exists()) rootDir.mkdirs();

        //파일명을 유니크하게 변경(이름 충돌 가능성 대비)
        //UUID와 원본파일명을 결합 -> 규칙은 없음
    ...
    public String findProfilePath(String userId) {
        ...
        //DB에는 파일명만 저장. -> service가 가지고 있는 Root Path와 연결해서 리턴(실제 로컬)
        return uploadRootPath + "/" + user.getProfileImg(); //왜 String ??? 리턴
    ...
    private KakaoUserDTO getKakaoUserInfo(String accessToken) {
        //요청 uri
        String requestURI = "https://kapi.kakao.com/v2/user/me"; //오타 @@

        //요청 헤더 설정
        HttpHeaders headers = new HttpHeaders(); //@@ 객체 생성(기본)
    ...
    private String getKakaoAccessToken(String code) {
        ...
        //헤더와 바디 정보를 합치기 위해 HttpEntity 객체생성
        HttpEntity<Object> requestEntity = new HttpEntity<>(params, headers); //순서 가만히 있으면 제공됨
        ...
        // 만약 구조가 복잡한 경우에는 응답 데이터 타입을 String으로 받아서 JSON-simple 라이브러리로 직접 해체. : 기상청 라이브러리 때 해봤음, Gson, jackson-data
        ...
        */
        ...
        Map<String, Object> responseData = (Map<String, Object>) responseEntity.getBody(); //대부분 : 키 값 string , 객체 있을 수도 있으니 Object, getBody를 Map으로 형변환!
        log.info("토큰 요청 응답 데이터 : {}", responseData);

        //여러가지 데이터 중 access_token이라는 이름의 데이터를 리턴
        //Object를 String으로 형 변환해서 리턴
        return (String) responseData.get("access_token"); //access_token만 뽑아서 리턴

    }

    public String logout(TokenUserInfo userInfo) {
        //accessToken이 null이 아니라면 카카오 로그인을 한 얘겠지?
    ...
    public String renewalAccessToken(Map<String, String> tokenRequest) {
        String refreshToken = tokenRequest.get("refreshToken"); //어디서 get //@@@
        ...
        if(isValid){
            ...
            //만료일이 오늘보다 이전이 아니라면 -> 만료되지 않았다면 20240610 // 20240524
